...
const cartesianRightRecursive = (arr) => (arr.length === 0
    ...
    : Array.isArray(arr[0])
        ...
        : isRecursiveTuple(arr)
            ? cartesianRightRecursive(arr[1]).map((ele) => [arr[0], ele])
            : arr);
const toMapping = (mapping, shape) => {
    ...
    const definedShape = (shape === undefined)
        ? [(0, array_1.range)(crr[0].length - 1), [crr[0].length - 1]]
    ...
    const buildMappingRecursively = (keyCartesianSet, values) => {
        ...
        const droppedKeyCol = (0, array_1.zip)(keyCartesianSet.slice(1));
        return Object.fromEntries(distinctKeys.map(key => {
            const rows = keyRows.get(key);
            const keyCartesianSubset = (0, array_1.zip)(rows.map(i => droppedKeyCol[i]));
    ...
    const ret = buildMappingRecursively(keyCartesianSet, (0, array_1.zip)(leafValues));
    if (allSingletons)
...
const has = (f) => (...args) => f(...args) !== undefined;
const get = (f) => (...args) => f(...args);
