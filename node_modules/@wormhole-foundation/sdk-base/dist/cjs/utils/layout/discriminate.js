...
exports.layoutDiscriminator = void 0;
...
function layoutItemMeta(item, offset, fixedBytes) {
    function knownFixed(size, serialized) {
        if (offset !== null)
...
//  respective layout.
function generateLayoutDiscriminator(layouts) {
    if (layouts.length === 0)
    ...
    const sizePower = layouts.length - Math.max(...[...ascendingBounds.values()].map(candidates => count(candidates)));
    ...
    const fixedKnownBytes = Array.from({ length: Math.max(...fixedKnown.map(fkb => fkb.length > 0 ? fkb.at(-1)[0] + fkb.at(-1)[1].length : 0))
    }).map(() => []);
    ...
    for (const [bytePos, fixedKnownByte] of fixedKnownBytes.entries()) {
        ...
        if (power === layouts.length - 1)
            ...
            return [
                true,
                (encoded) => bitsetToArray(encoded.length <= bytePos
                    ? outOfBoundsLayouts
    ...
    const recursivelyBuildStrategy = (candidates, bestBytes) => {
        if (count(candidates) <= 1 || strategies.has(candidates))
            return;
        let sizePower = 0;
        const narrowedBounds = new Map();
        ...
        for (const [power, bytePos, outOfBoundsLayouts, distinctValues, anyValueLayouts] of bestBytes) {
            const narrowedDistinctValues = new Map();
    ...
    return [distinguishable, (encoded) => {
            ...
            for (let strategy = strategies.get(candidates); strategy !== "indistinguishable"; strategy = strategies.get(candidates) ?? findSmallestSuperSetStrategy(candidates)) {
                if (strategy === "size")
...
function layoutDiscriminator(layouts, allowAmbiguous) {
...
exports.layoutDiscriminator = layoutDiscriminator;
