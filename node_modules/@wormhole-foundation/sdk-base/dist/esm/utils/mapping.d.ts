import { IndexEs, Flatten, InnerFlatten, IsRectangular, Zip, Cartesian, OnlyIndexes, ExcludeIndexes, Entries } from "./array";
...
type ProcessNextKeyColmn<KC extends CartesianSet<MappableKey>, VR extends RoArray> = KC["length"] extends 0 ? VR : ExcludeIndexes<KC, 0> extends infer KCR extends CartesianSet<MappableKey> ? ToMapEntries<Entries<KC[0]>> extends infer KRIA extends MappingEntries<RoArray<number>> ? [
    ...{
        [K in keyof KRIA]: [
            KRIA[K][0],
...
type ObjectFromMappingEntries<M extends MappingEntries, D extends Depth[number]> = {
    [K in keyof M as (K extends `${number}` ? ToExtPropKey<M[K][0]> : never)]: M[K][1] extends infer V ? D extends 1 ? V extends LeafValue<infer T> ? T : V extends RoArray<LeafValue> ? [...{
        [K2 in keyof V]: K2 extends `${number}` ? V[K2][1] : never;
    }] : V : V extends MappingEntries ? ObjectFromMappingEntries<V, Depth[D]> : never : never;
};
...
type RecursiveAccess<M extends Mapped, KA extends RoArray<MappableKey>> = KA extends readonly [infer Head extends MappableKey, ...infer Tail extends RoArray<MappableKey>] ? ToExtPropKey<Head> extends keyof M ? M[ToExtPropKey<Head>] extends Mapped ? RecursiveAccess<M[ToExtPropKey<Head>], Tail> : M[ToExtPropKey<Head>] : never : M;
type GenericMappingFunc<M extends Mapped, D extends number> = D extends 1 ? <K1 extends FromExtPropKey<keyof M>>(...args: [K1]) => RecursiveAccess<M, [K1]> : D extends 2 ? <K1 extends FromExtPropKey<keyof M>, K2 extends FromExtPropKey<keyof RecursiveAccess<M, [K1]>>>(...args: [K1, K2]) => RecursiveAccess<M, [K1, K2]> : D extends 3 ? <K1 extends FromExtPropKey<keyof M>, K2 extends FromExtPropKey<keyof RecursiveAccess<M, [K1]>>, K3 extends FromExtPropKey<keyof RecursiveAccess<M, [K1, K2]>>>(...args: [K1, K2, K3]) => RecursiveAccess<M, [K1, K2, K3]> : D extends 4 ? <K1 extends FromExtPropKey<keyof M>, K2 extends FromExtPropKey<keyof RecursiveAccess<M, [K1]>>, K3 extends FromExtPropKey<keyof RecursiveAccess<M, [K1, K2]>>, K4 extends FromExtPropKey<keyof RecursiveAccess<M, [K1, K2, K3]>>>(...args: [K1, K2, K3, K4]) => RecursiveAccess<M, [K1, K2, K3, K4]> : never;
type ToGenericMappingFunc<M extends MappingEntries, S extends Shape | undefined = undefined> = ToMappingAndDepth<M, S> extends [infer TM extends Mapped, infer D extends Depth[number]] ? GenericMappingFunc<TM, D> : never;
type WidenArray<T extends RoArray> = T extends readonly [infer Head, ...infer Tail extends RoArray] ? [Widen<Head>, ...WidenArray<Tail>] : [];
...
type ConstMapRet<F extends Function> = F & {
    get: Get<F>;
    has: Has<F>;
};
export declare function constMap<const M extends MappingEntries, const S extends Shape | undefined = undefined>(mappingEntries: M, shape?: S): ConstMapRet<ToGenericMappingFunc<M, S>>;
export {};
