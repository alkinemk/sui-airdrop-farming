import { zip, constMap } from "../utils";
const circleAPIs = [
...
export const isCircleChain = (chain) => circleChainId.has(chain);
export const isCircleChainId = (chainId) => circleChainIdToChain.has(chainId);
export const isCircleSupported = (network, chain) => usdcContract.has(network, chain);
export function assertCircleChainId(chainId) {
    if (!isCircleChainId(chainId))
        throw Error(`Unknown Circle chain id: ${chainId}`);
}
export function assertCircleChain(chain) {
    if (!isCircleChain(chain))
        throw Error(`Unknown Circle chain: ${chain}`);
}
//safe assertion that allows chaining
export const asCircleChainId = (chainId) => {
    assertCircleChainId(chainId);
    return chainId;
};
export const toCircleChainId = (chain) => {
    switch (typeof chain) {
        case "string":
            if (isCircleChain(chain))
                return circleChainId(chain);
            break;
        case "number":
            if (isCircleChainId(chain))
                return chain;
            break;
        case "bigint":
            const ci = Number(chain);
            if (isCircleChainId(ci))
                return ci;
            break;
    }
    throw Error(`Cannot convert to ChainId: ${chain}`);
};
export const toCircleChain = (chain) => {
    switch (typeof chain) {
        case "string":
            if (isCircleChain(chain))
                return chain;
        ...
        case "number":
            if (isCircleChainId(chain))
                return circleChainIdToChain(chain);
            break;
        case "bigint":
            const cid = Number(chain);
            if (isCircleChainId(cid))
                return circleChainIdToChain(cid);
            break;
    }
