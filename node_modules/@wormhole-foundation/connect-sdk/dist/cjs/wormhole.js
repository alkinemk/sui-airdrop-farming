...
class Wormhole {
    ...
    async circleTransfer(amount, from, to, automatic, payload, nativeGas) {
        ...
        if (!sdk_base_1.circle.isCircleChain(from.chain) ||
            !sdk_base_1.circle.isCircleChain(to.chain) ||
     ...
     * @throws Errors if the VAA is not available after the retries
     */
    async getVaa(wormholeMessageId, decodeAs, timeout = config_1.DEFAULT_TASK_TIMEOUT) {
        return await (0, whscan_api_1.getVaaWithRetry)(this.config.api, wormholeMessageId, decodeAs, timeout);
    }
    /**
     * Gets a VAA from the API or Guardian RPC, finality must be met before the VAA will be available.
     ...
     * @throws Errors if the VAA is not available after the retries
     */
    async getVaaByTxHash(txid, decodeAs, timeout = config_1.DEFAULT_TASK_TIMEOUT) {
        return await (0, whscan_api_1.getVaaByTxHashWithRetry)(this.config.api, txid, decodeAs, timeout);
    }
    /**
     ...
     */
    async getTransactionStatus(wormholeMessageId, timeout = config_1.DEFAULT_TASK_TIMEOUT) {
        return (0, whscan_api_1.getTransactionStatusWithRetry)(this.config.api, wormholeMessageId, timeout);
    }
    /**
