...
class TokenTransfer {
    ...
    async completeTransfer(signer) {
        ...
        this._state = wormholeTransfer_1.TransferState.DestinationInitiated;
    ...
    static async validateTransferDetails(wh, transfer, fromChain, toChain) {
    ...
    static async quoteTransfer(srcChain, dstChain, transfer) {
        ...
        const srcToken = transfer.token === "native" ? await srcChain.getNativeWrappedTokenId() : transfer.token;
        ...
        const dedust = (amt) => scale(truncate(amt, srcDecimals), srcDecimals);
        // truncate and rescale to fit destination token
        const mintable = (amt) => scale(truncate(amt, srcDecimals), dstDecimals);
