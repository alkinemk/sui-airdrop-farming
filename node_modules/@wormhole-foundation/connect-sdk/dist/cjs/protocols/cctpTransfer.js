...
class CircleTransfer {
    ...
    constructor(wh, transfer, fromChain, toChain) {
        this._state = wormholeTransfer_1.TransferState.Created;
        this.wh = wh;
    ...
    static async from(wh, from, timeout = config_1.DEFAULT_TASK_TIMEOUT, fromChain, toChain) {
    ...
    static async fromWormholeMessageId(wh, from, timeout) {
        ...
        const rcvChain = sdk_base_1.circle.toCircleChain(vaa.payload.targetDomain);
        ...
        let automatic = false;
        if (wormholeRelayer) {
            const relayerAddress = wormhole_1.Wormhole.chainAddress(chain, wormholeRelayer).address.toUniversalAddress();
        ...
        tt._state = wormholeTransfer_1.TransferState.Attested;
        return tt;
    }
    static async fromCircleMessage(wh, message) {
        ...
        const sendChain = sdk_base_1.circle.toCircleChain(msg.sourceDomain);
        const rcvChain = sdk_base_1.circle.toCircleChain(msg.destinationDomain);
        const details = {
        ...
        xfer._state = wormholeTransfer_1.TransferState.SourceInitiated;
        return xfer;
    }
    ...
    static async fromTransaction(wh, from, timeout, fromChain) {
        ...
        // transfer
        const msgIds = await fromChain.parseTransaction(txid);
        ...
        ct._state = wormholeTransfer_1.TransferState.SourceInitiated;
        ct.txids = [from];
        return ct;
    }
    ...
    async initiateTransfer(signer) {
        ...
        */
        if (this._state !== wormholeTransfer_1.TransferState.Created)
            throw new Error("Invalid state transition in `start`");
        let xfer;
        if (this.transfer.automatic) {
            const cr = await this.fromChain.getAutomaticCircleBridge();
            xfer = cr.transfer(this.transfer.from.address, { chain: this.transfer.to.chain, address: this.transfer.to.address }, this.transfer.amount, this.transfer.nativeGas);
        }
        else {
            const cb = await this.fromChain.getCircleBridge();
            xfer = cb.transfer(this.transfer.from.address, { chain: this.transfer.to.chain, address: this.transfer.to.address }, this.transfer.amount);
        }
        this.txids = await (0, common_1.signSendWait)(this.fromChain, xfer, signer);
        this._state = wormholeTransfer_1.TransferState.SourceInitiated;
    ...
    async fetchAttestation(timeout) {
        /*
            ...
            3) return seq
        */
        if (this._state < wormholeTransfer_1.TransferState.SourceInitiated || this._state > wormholeTransfer_1.TransferState.Attested)
        ...
        this._state = wormholeTransfer_1.TransferState.Attested;
        return ids;
    }
    ...
    async completeTransfer(signer) {
        ...
        */
        if (this._state < wormholeTransfer_1.TransferState.Attested)
        ...
        const txids = await (0, common_1.signSendWait)(this.toChain, xfer, signer);
    ...
    static async quoteTransfer(srcChain, dstChain, transfer) {
        ...
        const dstToken = wormhole_1.Wormhole.chainAddress(dstChain.chain, dstUsdcAddress);
        const srcToken = wormhole_1.Wormhole.chainAddress(srcChain.chain, srcUsdcAddress);
    ...
    static async isTransferComplete(toChain, attestation) {
    ...
    static async getTransferVaa(wh, wormholeMessageId, timeout) {
    ...
    static async getTransferMessage(fromChain, txid) {
        const cb = await fromChain.getCircleBridge();
        const circleMessage = await cb.parseTransactionDetails(txid);
    ...
    static getReceipt(xfer) {
        ...
        let receipt = {
            ...
            from: from.chain,
            to: to.chain,
            state: wormholeTransfer_1.TransferState.Created,
        };
        const originTxs = xfer.txids.filter((txid) => txid.chain === xfer.transfer.from.chain);
        if (originTxs.length > 0) {
            receipt = {
                ...receipt,
                state: wormholeTransfer_1.TransferState.SourceInitiated,
                originTxs,
            };
        }
        ...
        if (attestation) {
            if (attestation.id) {
                ...
                if (attestation.attestation) {
                    receipt = {
                        ...receipt,
                        state: wormholeTransfer_1.TransferState.Attested,
        ...
        if (destinationTxs.length > 0) {
            receipt = {
                ...receipt,
                state: wormholeTransfer_1.TransferState.DestinationInitiated,
                destinationTxs,
            };
        }
        return receipt;
    ...
    static async *track(wh, receipt, timeout = config_1.DEFAULT_TASK_TIMEOUT, 
    ...
    _fromChain, _toChain) {
        ...
        if ((0, wormholeTransfer_1.isSourceInitiated)(receipt)) {
        ...
        if ((0, wormholeTransfer_1.isAttested)(receipt) || (0, wormholeTransfer_1.isSourceFinalized)(receipt)) {
            if (!receipt.attestation)
            ...
            if (txStatus && txStatus.globalTx?.destinationTx?.txHash) {
                ...
                receipt = {
                    ...
                    state: wormholeTransfer_1.TransferState.DestinationFinalized,
                };
                yield receipt;
            }
        }
        ...
        if ((0, wormholeTransfer_1.isAttested)(receipt)) {
