...
class GatewayTransfer {
    ...
    static async from(wh, from, timeout) {
        ...
        gt._state = wormholeTransfer_1.TransferState.SourceInitiated;
    ...
    static async _fromMsgId(wh, from, timeout) {
        ...
        if (payload) {
            try {
                ...
                to = wormhole_1.Wormhole.chainAddress(destChain, recipientAddress);
            }
            catch {
    ...
    async fetchAttestation(timeout) {
        ...
        if (this.fromGateway()) {
            ...
            if (!this.toGateway()) {
                ...
                const whm = await (0, tasks_1.retry)(task, retryInterval, timeout, "Gateway:IbcBridge:LookupWormholeMessageFromIncomingIbcMessage");
                if (!whm)
            ...
            const [whm] = await wormhole_1.Wormhole.parseMessageFromTx(chain, transferTransaction.txid);
            const vaa = await GatewayTransfer.getTransferVaa(this.wh, whm);
            this.vaas = [{ id: whm, vaa }];
            attestations.push(whm);
            ...
            const isRedeemedTask = () => (0, tasks_1.isTokenBridgeVaaRedeemed)(wcTb, vaa);
            const redeemed = await (0, tasks_1.retry)(isRedeemedTask, vaaRedeemedRetryInterval, timeout, "Gateway:TokenBridge:IsVaaRedeemed");
            if (!redeemed)
            ...
            const destTransfer = await (0, tasks_1.retry)(destTransferTask, transferCompleteInterval, timeout, "Destination:IbcBridge:WormchainTransferCompleted");
            if (!destTransfer)
    ...
    async completeTransfer(signer) {
        ...
        */
        ...
        const redeemTxs = await (0, common_1.signSendWait)(toChain, xfer, signer);
    ...
    static async getTransferVaa(wh, whm, timeout) {
