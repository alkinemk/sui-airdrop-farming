    ...
    static async supportedSourceTokens(fromChain) {
        ...
        return supported.map((td) => {
            if (td.address === "native")
                return "native";
            return __1.Wormhole.chainAddress(fromChain.chain, td.address);
        });
    }
    static async supportedDestinationTokens(sourceToken, fromChain, toChain) {
        const tokenAddress = (0, __1.canonicalAddress)(sourceToken);
        ...
        const locallyRedeemable = (__1.tokens.filters.bySymbol(toChain.config.tokenMap, redeemTokenDetails.symbol) ?? [])
            ...
            switch (td.symbol) {
                case "ETH":
                case "WETH":
                    return __1.Wormhole.chainAddress(toChain.chain, td.address);
                case "WSTETH":
                    return __1.Wormhole.chainAddress(toChain.chain, td.address);
                default:
