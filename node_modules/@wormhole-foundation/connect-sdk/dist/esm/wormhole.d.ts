...
type ChainMap<N extends Network, C extends Chain = Chain> = Map<C, ChainContext<N, ChainToPlatform<C>, C>>;
export declare class Wormhole<N extends Network> {
     ...
     * @throws Errors if the chain or protocol is not supported
     */
    tokenTransfer(token: TokenId | "native", amount: bigint, from: ChainAddress, to: ChainAddress, automatic: boolean, payload?: Uint8Array, nativeGas?: bigint): Promise<TokenTransfer<N>>;
    /**
     ...
     */
    getDecimals<C extends Chain>(chain: C, token: TokenAddress<C>): Promise<bigint>;
    /**
     ...
     * @throws Errors if the VAA is not available after the retries
     */
    getVaa<T extends PayloadLiteral | PayloadDiscriminator>(wormholeMessageId: WormholeMessageId, decodeAs: T, timeout?: number): Promise<ReturnType<typeof deserialize<T>> | null>;
    /**
     * Gets a VAA from the API or Guardian RPC, finality must be met before the VAA will be available.
     ...
     * @throws Errors if the VAA is not available after the retries
     */
    getVaaByTxHash<T extends PayloadLiteral | PayloadDiscriminator>(txid: TxHash, decodeAs: T, timeout?: number): Promise<ReturnType<typeof deserialize<T>> | null>;
    /**
     ...
     */
    getTransactionStatus(wormholeMessageId: WormholeMessageId, timeout?: number): Promise<TransactionStatus | null>;
    /**
     ...
     */
    static parseMessageFromTx<N extends Network, P extends Platform, C extends PlatformToChains<P>>(chain: ChainContext<N, P, C>, txid: TxHash, timeout?: number): Promise<WormholeMessageId[]>;
}
export {};
