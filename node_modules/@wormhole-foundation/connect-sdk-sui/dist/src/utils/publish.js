"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.publishPackage = exports.getCoinBuildOutput = exports.publishCoin = void 0;
const sui_js_1 = require("@mysten/sui.js");
const utils_1 = require("./utils");
const publishCoin = async (provider, coreBridgeStateObjectId, tokenBridgeStateObjectId, decimals, signerAddress, coreBridgePackageId, tokenBridgePackageId) => {
    [coreBridgePackageId, tokenBridgePackageId] = await Promise.all([
        coreBridgePackageId
            ? Promise.resolve(coreBridgePackageId)
            : (0, utils_1.getPackageId)(provider, coreBridgeStateObjectId),
        tokenBridgePackageId
            ? Promise.resolve(tokenBridgePackageId)
            : (0, utils_1.getPackageId)(provider, tokenBridgeStateObjectId),
    ]);
    const build = await (0, exports.getCoinBuildOutput)(provider, coreBridgePackageId, tokenBridgePackageId, tokenBridgeStateObjectId, decimals);
    return (0, exports.publishPackage)(build, signerAddress);
};
exports.publishCoin = publishCoin;
const getCoinBuildOutput = async (provider, coreBridgePackageId, tokenBridgePackageId, tokenBridgeStateObjectId, decimals) => {
    // Decimals is capped at 8
    decimals = Math.min(decimals, 8);
    // Construct bytecode, parametrized by token bridge package ID and decimals
    const strippedTokenBridgePackageId = (await (0, utils_1.getOriginalPackageId)(provider, tokenBridgeStateObjectId))?.replace('0x', '');
    if (!strippedTokenBridgePackageId) {
        throw new Error(`Original token bridge package ID not found for object ID ${tokenBridgeStateObjectId}`);
    }
    const bytecodeHex = '' +
        strippedTokenBridgePackageId +
        '00020106010000000001090b0031' +
        decimals.toString(16).padStart(2, '0') +
        '0a0138000b012e110238010200';
    const bytecode = Buffer.from(bytecodeHex, 'hex').toString('base64');
    return {
        modules: [bytecode],
        dependencies: ['0x1', '0x2', tokenBridgePackageId, coreBridgePackageId].map((d) => (0, sui_js_1.normalizeSuiObjectId)(d)),
    };
};
exports.getCoinBuildOutput = getCoinBuildOutput;
const publishPackage = async (buildOutput, signerAddress) => {
    // Publish contracts
    const tx = new sui_js_1.TransactionBlock();
    const [upgradeCap] = tx.publish({
        modules: buildOutput.modules.map((m) => Array.from((0, sui_js_1.fromB64)(m))),
        dependencies: buildOutput.dependencies.map((d) => (0, sui_js_1.normalizeSuiObjectId)(d)),
    });
    // Transfer upgrade capability to recipient
    tx.transferObjects([upgradeCap], tx.pure(signerAddress));
    return tx;
};
exports.publishPackage = publishPackage;
//# sourceMappingURL=publish.js.map